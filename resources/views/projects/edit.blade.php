        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6 py-2">
            <div>
                <h2 class="font-black text-3xl text-black leading-tight tracking-tighter">
                    {{ __('Edit Project') }}: <span class="text-indigo-800">{{ $project->name }}</span>
                </h2>
                <p class="mt-1 text-base font-bold text-slate-800">
                    {{ __('Modify project details and keep information up to date.') }}
                </p>
            </div>
            <nav class="flex" aria-label="Breadcrumb">
                <ol class="inline-flex items-center bg-white px-5 py-3 rounded-xl border-2 border-slate-400 shadow-sm">
                    ...
                    <li class="flex items-center">
                        ...
                        <a href="{{ route('projects.index') }}" class="text-xs font-black uppercase tracking-widest text-slate-700 hover:text-indigo-800 transition-colors">{{ __('Projects') }}</a>
                    </li>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-transparent overflow-hidden">
                <form action="{{ route('projects.update', $project) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    @include('projects.partials.form', ['submitText' => __('Update Project')])
                </form>
            </div>
        </div>
    </div>
