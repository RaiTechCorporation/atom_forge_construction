        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div>
                ...
                <p class="mt-1 text-sm font-medium text-slate-500">
                    {{ __('Define a new access level and assign module permissions.') }}
                </p>
            </div>
            <div>
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div class="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        ...
                        <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name')" required placeholder="e.g. Site Supervisor" />
                        <x-input-error class="mt-2" :messages="$errors->get('name')" />
                    </div>
                    <div>
                        <x-input-label for="description" :value="__('Description')" />
                        <x-text-input id="description" name="description" type="text" class="mt-1 block w-full" :value="old('description')" placeholder="Briefly describe what this role can do" />
                        <x-input-error class="mt-2" :messages="$errors->get('description')" />
                    </div>
                    <div class="flex items-center gap-2">
                        <input type="checkbox" name="is_active" id="is_active" value="1" checked class="rounded border-slate-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
                        ...
                        <div class="p-6 bg-slate-50/50 border border-slate-100 rounded-2xl space-y-4">
                            ...
                            <div class="space-y-3">
                                ...
                                <label class="flex items-center gap-3 group cursor-pointer">
                                    <input type="checkbox" name="permissions[]" value="{{ $permission->id }}" class="permission-checkbox module-{{ Str::slug($module) }} rounded border-slate-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
