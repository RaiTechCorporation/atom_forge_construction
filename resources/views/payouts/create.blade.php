        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6 py-2">
            <div>
                ...
                <p class="mt-1 text-base font-bold text-slate-800">
                    {{ __('Log a new return distribution to an investor.') }}
                </p>
            </div>
            <nav class="flex" aria-label="Breadcrumb">
                <ol class="inline-flex items-center bg-white px-5 py-3 rounded-xl border-2 border-slate-400 shadow-sm">
                    ...
                    <li class="flex items-center">
                        ...
                        <a href="{{ route('payouts.index') }}" class="text-xs font-black uppercase tracking-widest text-slate-700 hover:text-indigo-800 transition-colors">{{ __('Payouts') }}</a>
                    </li>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-transparent overflow-hidden">
                <form action="{{ route('payouts.store') }}" method="POST">
                    @csrf
                    @include('payouts.partials.form', ['submitText' => __('Record Payout')])
                </form>
            </div>
        </div>
    </div>
