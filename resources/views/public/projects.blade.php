    ...
    <div x-data="{ selectedType: 'All' }">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center mb-16">
                <div class="flex flex-wrap justify-center gap-3">
                    <button 
                        ...
                        :class="selectedType === 'All' ? 'bg-orange-primary text-white shadow-orange-500/20' : 'bg-slate-50 text-slate-600 border-slate-100 hover:bg-slate-100'"
                        class="px-6 py-2 rounded-xl font-bold transition-all border shadow-lg">All Projects</button>
                    <button 
                        @click="selectedType = 'Residential'" 
                        :class="selectedType === 'Residential' ? 'bg-orange-primary text-white shadow-orange-500/20' : 'bg-slate-50 text-slate-600 border-slate-100 hover:bg-slate-100'"
                        class="px-6 py-2 rounded-xl font-bold transition-all border shadow-lg">Residential</button>
                    <button 
                        @click="selectedType = 'Commercial'" 
                        :class="selectedType === 'Commercial' ? 'bg-orange-primary text-white shadow-orange-500/20' : 'bg-slate-50 text-slate-600 border-slate-100 hover:bg-slate-100'"
                        class="px-6 py-2 rounded-xl font-bold transition-all border shadow-lg">Commercial</button>
                    <button 
                        @click="selectedType = 'Interiors'" 
                        :class="selectedType === 'Interiors' ? 'bg-orange-primary text-white shadow-orange-500/20' : 'bg-slate-50 text-slate-600 border-slate-100 hover:bg-slate-100'"
                        class="px-6 py-2 rounded-xl font-bold transition-all border shadow-lg">Interiors</button>
                </div>
            </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                ...
                <div 
                    x-show="selectedType === 'All' || selectedType === '{{ $p['type'] }}'"
                    x-transition:enter="transition ease-out duration-300"
                    x-transition:enter-start="opacity-0 transform scale-90"
                    x-transition:enter-end="opacity-100 transform scale-100"
                    class="group bg-white rounded-[2.5rem] overflow-hidden border border-slate-100 shadow-sm hover:shadow-2xl hover:shadow-slate-200/50 transition-all duration-500">
                    <div class="relative overflow-hidden aspect-[4/3]">
                        ...
                        <div class="absolute top-6 left-6">
                            <span class="px-3 py-1 bg-white/90 backdrop-blur-sm rounded-lg text-[10px] font-bold uppercase tracking-widest text-orange-primary shadow-sm">{{ $p['type'] }}</span>
                        </div>
                    </div>
                    <div class="p-8">
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <a href="#" class="w-10 h-10 rounded-full bg-slate-50 flex items-center justify-center text-slate-900 hover:bg-orange-primary hover:text-white transition-all">
    ...
    <section class="py-24 bg-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-slate-50 rounded-[3rem] p-12 md:p-20 text-center border border-slate-100">
                ...
                <a href="{{ route('contact') }}" class="inline-flex items-center px-10 py-4 bg-orange-primary text-white font-bold rounded-xl shadow-lg shadow-orange-500/20 hover:opacity-90 transition-all">Start Your Journey</a>
            </div>
        </div>
    </section>
@endsection
