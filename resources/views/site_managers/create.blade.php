<x-app-layout>
        ...
        <div class="flex flex-col md:flex-row justify-between items-center gap-6">
            <div>
                <h2 class="font-bold text-2xl text-slate-900 leading-tight tracking-tight">
                    {{ __('Register Site Manager') }}
                </h2>
                <p class="mt-1 text-sm font-medium text-slate-500">
                    {{ __('Onboard a new site manager to the management system.') }}
                </p>
            </div>
    ...
    <div class="max-w-7xl mx-auto">
        <form action="{{ route('site-managers.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            @csrf
            
            @include('site_managers.partials.form')

            <div class="flex justify-end gap-4 pb-12">
                <button type="reset" class="px-6 py-3 bg-white border border-slate-200 text-slate-700 font-black uppercase tracking-widest text-[10px] rounded-xl hover:bg-slate-50 transition-all">
                    Reset Form
                </button>
