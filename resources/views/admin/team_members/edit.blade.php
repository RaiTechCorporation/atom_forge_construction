        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div>
                ...
                <p class="mt-1 text-sm font-medium text-slate-500">
                    {{ __('Update the details for ') }} <span class="text-indigo-600 font-bold">{{ $teamMember->name }}</span>
                </p>
            </div>
            <a href="{{ route('team-members.index') }}" class="group relative px-6 py-3 bg-white text-slate-900 border border-slate-200 font-black uppercase tracking-widest text-[11px] rounded-xl hover:bg-slate-50 transition-all duration-300 shadow-sm">
            ...
            <div class="space-y-8">
                <div class="premium-card overflow-hidden">
                    <div class="p-8 border-b border-slate-100 bg-slate-50/30 flex items-center gap-3">
                        ...
                        <h3 class="text-lg font-black text-slate-900 uppercase tracking-wider">Update Member Details</h3>
                    </div>
                    
                    <div class="p-8 space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                <label for="name" class="stat-label">Full Name</label>
                                <input 
                                    type="text" 
                                    id="name" 
                                    name="name" 
                            ...
                            <div class="space-y-2">
                                <label for="role" class="stat-label">Role / Designation</label>
                                <input 
                                    type="text" 
                                    id="role" 
                                    name="role" 
                                ...
                                >
                                @error('role') <p class="text-xs font-bold text-rose-600 mt-1">{{ $message }}</p> @enderror
                            </div>
                        </div>

                        <div class="space-y-2">
                            <label for="bio" class="stat-label">Biography</label>
                            <textarea 
                                id="bio" 
                                name="bio" 
                                rows="5" 
                                class="input-premium"
                                placeholder="Enter a brief biography of the team member..."
                            >{{ old('bio', $teamMember->bio) }}</textarea>
                            @error('bio') <p class="text-xs font-bold text-rose-600 mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div class="space-y-4">
                            <label class="stat-label">Profile Image</label>
                            
                            @if($teamMember->image_url)
                                <div class="relative w-24 h-24 rounded-2xl overflow-hidden border-2 border-slate-100 shadow-sm group/thumb">
                                    <img src="{{ $teamMember->image_url }}" class="w-full h-full object-cover">
                                    <div class="absolute inset-0 bg-black/40 opacity-0 group-hover/thumb:opacity-100 transition-opacity flex items-center justify-center">
                                        <span class="text-[8px] font-black text-white uppercase tracking-widest">Current Photo</span>
                                    </div>
                                </div>
                            @endif
                            ...
                            <div class="relative group/file">
                                ...
                                <div class="w-full px-5 py-4 bg-white border border-slate-200 rounded-xl flex items-center gap-3 group-hover/file:border-indigo-600 transition-all shadow-sm">
                                    ...
                                    <span class="text-sm font-bold text-slate-500">
                                        {{ $teamMember->image_url ? 'Upload new photo to replace' : 'Choose profile photo or drag & drop' }}
                                    </span>
                                </div>
                            </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="flex items-center pt-8">
                                <label class="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ old('is_active', $teamMember->is_active) ? 'checked' : '' }}>
