        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6 py-2">
            <div>
                <h2 class="font-black text-3xl text-slate-900 leading-tight tracking-tight">
                    {{ __('Onboard New Investor') }}
                </h2>
                <p class="mt-1 text-base font-bold text-slate-500">
                    {{ __('Register a new partner and create their portal credentials.') }}
                </p>
            </div>
            <nav class="flex" aria-label="Breadcrumb">
                <ol class="inline-flex items-center bg-white px-5 py-3 rounded-2xl border border-slate-100 shadow-sm">
                    ...
                    <li class="flex items-center" aria-current="page">
                        ...
                        <span class="text-xs font-black uppercase tracking-widest text-indigo-600">{{ __('Create') }}</span>
                    </li>
                </ol>
            </nav>
        </div>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form action="{{ route('investors.store') }}" method="POST">
                @csrf
                @include('admin.investors.partials.form', ['submitText' => __('Register Investor')])
            </form>
        </div>
    </div>
</x-app-layout>
