            ...
            <nav class="space-y-1">
                @if(!$isInvestorMode)
                    ...
                    @foreach($links as $link)
                        @if($link['route'] === 'admin.blogs.index' && auth()->user()->isSiteSupervisor())
                            @continue
                        @endif
        ...
        <div>
            ...
            <nav class="space-y-1">
                ...
                @endunless
                    ...
                    @if(auth()->user()->hasPermission('view-employees') || auth()->user()->isSiteSupervisor())
                    <!-- Labour Dropdown -->
                    ...
                    <div x-data="{ managersOpen: {{ request()->routeIs('site-managers.*') || request()->routeIs('labour.work-progress.*') ? 'true' : 'false' }} }" class="mt-1">
                        <button @click="managersOpen = !managersOpen" 
                                class="w-full flex items-center justify-between gap-3 px-4 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all {{ request()->routeIs('site-managers.*') || request()->routeIs('labour.work-progress.*') ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-600/30' : 'text-slate-400 hover:bg-white/5 hover:text-white' }}">
                            <div class="flex items-center gap-3">
                                <span class="{{ request()->routeIs('site-managers.*') || request()->routeIs('labour.work-progress.*') ? 'text-white' : 'text-slate-600' }}">
        ...
        <div class="px-4 py-3 bg-slate-900/50 border border-slate-800/50 rounded-2xl flex items-center gap-4">
            ...
            <div class="flex flex-col min-w-0">
                ...
                <span class="text-[9px] font-black text-slate-500 uppercase tracking-widest mt-1">
                    {{ Auth::user()->isSuperAdmin() ? 'Systems Lead' : (Auth::user()->isSiteSupervisor() ? 'Site Supervisor' : (Auth::user()->isAdminStaff() ? 'Operations Staff' : (Auth::user()->isInvestor() ? 'Capital Partner' : 'My Account'))) }}
                </span>
