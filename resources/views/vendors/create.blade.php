        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6 py-2">
            <div>
                ...
                <p class="mt-1 text-base font-bold text-slate-800">
                    {{ __('Onboard a new supplier or service provider to the system.') }}
                </p>
            </div>
            <nav class="flex" aria-label="Breadcrumb">
                <ol class="inline-flex items-center bg-white px-5 py-3 rounded-xl border-2 border-slate-400 shadow-sm">
                    ...
                    <li class="flex items-center" aria-current="page">
                        ...
                        <span class="text-xs font-black uppercase tracking-widest text-indigo-800">{{ __('Onboard') }}</span>
                    </li>
                </ol>
            </nav>
        </div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-transparent overflow-hidden">
                <form action="{{ route('vendors.store') }}" method="POST">
                    @csrf
                    @include('vendors.partials.form', ['submitText' => __('Add Vendor')])
                </form>
            </div>
        </div>
    </div>
