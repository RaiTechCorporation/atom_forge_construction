        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6 py-2">
            <div>
                ...
                <p class="mt-1 text-base font-bold text-slate-800">
                    {{ __('Register a new worker in the labour management system.') }}
                </p>
            </div>
            <nav class="flex" aria-label="Breadcrumb">
                <ol class="inline-flex items-center bg-white px-5 py-3 rounded-xl border-2 border-slate-400 shadow-sm">
                    ...
                    <li class="flex items-center">
                        ...
                        <a href="{{ route('labour.index') }}" class="text-xs font-black uppercase tracking-widest text-slate-700 hover:text-indigo-800 transition-colors">{{ __('Labour') }}</a>
                    </li>
                    <li class="flex items-center" aria-current="page">
                        <svg class="w-5 h-5 text-slate-400 mx-2" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path>
                        </svg>
                        <span class="text-xs font-black uppercase tracking-widest text-indigo-800">{{ __('Register') }}</span>
                    </li>
                </ol>
            </nav>
        </div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-transparent overflow-hidden">
                <form action="{{ route('labour.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @include('labour.partials.form', ['submitText' => __('Add Worker')])
                </form>
            </div>
        </div>
    </div>
