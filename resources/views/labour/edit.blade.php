        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6 py-2">
            <div>
                ...
                <p class="mt-1 text-base font-bold text-slate-800">
                    {{ __('Update details for :name', ['name' => $labour->name]) }}
                </p>
            </div>
    ...
    <div class="py-12 bg-white min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-transparent overflow-hidden">
                <form action="{{ route('labour.update', $labour) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    @include('labour.partials.form', ['submitText' => __('Update Worker')])
                </form>
            </div>
        </div>
    </div>
