        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6 py-2">
            <div>
                <h2 class="font-black text-3xl text-black leading-tight tracking-tighter">
                    {{ __('Edit Material') }}: <span class="text-indigo-800">{{ $material->name }}</span>
                </h2>
                <p class="mt-1 text-base font-bold text-slate-800">
                    {{ __('Update material name and measurement units.') }}
                </p>
            </div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-transparent overflow-hidden">
                <form action="{{ route('materials.update', $material) }}" method="POST">
                    @csrf
                    @method('PUT')
                    @include('materials.partials.form', ['submitText' => __('Update Material')])
                </form>
            </div>
        </div>
    </div>
